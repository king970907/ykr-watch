...
const Header: React.FC = () => {
  ...
  return (
    <header
      className={`bg-slate-950/80 backdrop-blur-md border-b ${themeConfig.border} sticky top-0 z-50 px-6 py-4`}
    >
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <div
            className={`${themeConfig.bg} p-2 rounded-lg shadow-lg shadow-${themeConfig.primary}-500/20`}
          >
        ...
        <nav className="flex bg-slate-800/50 p-1 rounded-xl border border-slate-700">
          <button
            ...
            className={`flex items-center gap-2 px-6 py-2 rounded-lg text-sm font-bold transition-all ${
              activeTab === "demo"
                ? `${themeConfig.bg} text-white shadow-lg`
